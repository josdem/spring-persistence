...
Habilitan trabajar con múltiples recursos transaccionales, tipicamente bases de datos relaciones y colas de mensajes. Los servidores de aplicaciones administran las transacciones a través de JTA. Sin embargo, una transacción manejada por JTA normalmente viene de una fuente en un JNDI, lo cual significa el uso de JNDI adicional al de JTA.
...
*AnnotatedTransactionsTests.java*
[source,java]
----
package com.makingdevs.practica10;
...
### MANDATORY
* Debe existir una transacción, si no existe falla y arroja excepeción
